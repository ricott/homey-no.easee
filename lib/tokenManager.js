    ...
    async getToken(username, password, device, force = false) {
        return await mutex.runExclusive(async () => {
            ...
            if (force) {
                ...
                device.logMessage(`Force refresh ignored - token is less than ${MIN_TOKEN_AGE}ms old`);
            }
    ...
    constructor(username, password, token, device) {
        ...
        this._timer = this._device.homey.setInterval(async () => {
            try {
                ...
                this._device.logMessage(`Failed to refresh tokens, generating new using user/password`);
                this._device.error(error);
                this._retryCount++;

                if (this._retryCount >= this._maxRetries) {
                    this._device.logMessage(`Max retries (${this._maxRetries}) reached, stopping refresh timer`);
                    this.cleanup();
                    return;
                }
                ...
                try {
                    ...
                    this._device.logMessage(`Failed to generate new tokens, will retry ${this._maxRetries - this._retryCount} more times`);
                    this._device.error(error);
                }
            }
