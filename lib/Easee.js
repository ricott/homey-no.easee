...
class Easee {
    ...
    */
    ...
    /*
        ...
        Attempts to start charging. If rejected or expired, will try to resume charging and/or override schedule as needed.
        Handles the quirks seen in Easee API/cloud.
    */
    async startChargingSmart() {
        this._logMessage('[startChargingSmart] Initiating smart charging for charger: ' + this.getData().id);

        // Step 1: Get current charger status from observation API
        let status;
        try {
            const state = await this.createEaseeChargerClient().getChargerState(this.getData().id);
            const statusObservation = state.find(obs => obs.id === 109); // Or use your enums mapping!
            status = enums.decodeChargerMode(statusObservation?.value);
            this._logMessage(`[startChargingSmart] Current charger status: ${status}`);
        } catch (err) {
            this._logMessage('[startChargingSmart] Could not determine current status, will try startCharging as fallback');
            status = null;
        }

        // Step 2: Smart action selection
        try {
            if (status === 'Paused') {
                this._logMessage('[startChargingSmart] Charger is paused, resuming...');
                await this.resumeCharging();
            } else if (status && status !== 'Charging') {
                this._logMessage('[startChargingSmart] Not charging, trying to override schedule...');
                await this.overrideSchedule();
                // Optional: Wait a bit for override to propagate, e.g., await this.#sleep(3000);
                await this.#sleep(2000);
            }
            // Check again if we are now charging
            const newState = await this.createEaseeChargerClient().getChargerState(this.getData().id);
            const newStatus = enums.decodeChargerMode(newState.find(obs => obs.id === 109)?.value);
            if (newStatus === 'Charging') {
                this._logMessage('[startChargingSmart] Charging started successfully!');
                return true;
            }
            // If still not charging, try the nuclear option
            this._logMessage('[startChargingSmart] Still not charging, trying startCharging() API call...');
            await this.startCharging();
            // Final check
            await this.#sleep(2000);
            const finalState = await this.createEaseeChargerClient().getChargerState(this.getData().id);
            const finalStatus = enums.decodeChargerMode(finalState.find(obs => obs.id === 109)?.value);
            if (finalStatus === 'Charging') {
                this._logMessage('[startChargingSmart] Charging started successfully after startCharging()!');
                return true;
            }
            throw new Error('[startChargingSmart] Could not start charging, all attempts failed');
        } catch (err) {
            this.logError('[startChargingSmart] Failed: ' + err);
            throw err;
        }
    }
...
async function invoke(domain, method, self, path, data) {
    ...
    // --- Diagnostic logging (always log method, url, and payload if present)
    self._logMessage('INFO', `Invoking HTTP: ${method} ${url}`);
    if (data) self._logMessage('INFO', `Payload: ${JSON.stringify(data)}`);
    ...
    try {
        ...
        if (statusCode >= 200 && statusCode < 300) {
                ...
                if (contentType && contentType.includes('application/json')) {
                    try {
                        ...
                        if (typeof json === 'object' && json !== null) {
                            ...
                            // If it's a number (or anything non-object), wrap it in an object
                            return { value: json, statusCode };
                        }
                    } catch (jsonError) {
            ...
            try {
                ...
                errorDetail = { error: 'Failed to read error body', details: err.message };
            }
            // Log all error details for diagnostics
            self._logMessage('ERROR', `HTTP ${statusCode} error on ${method} '${url}':`, errorDetail);
            throw new Error(`${method} '${path}': Failed, HTTP status code '${statusCode}' - Details: ${JSON.stringify(errorDetail)}`);
        }
    } catch (error) {
    ...
    while (!done && counter < maxRetries) {
        ...
        try {
            ...
            if (error.message.includes('404')) {
                ...
                self._logMessage('ERROR', `Full error: ${JSON.stringify(error, Object.getOwnPropertyNames(error))}`);
