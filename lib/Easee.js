...
class Easee {
    ...
    #postWaitForResponse(url, payload) {
        let self = this;
    ...
    */
    ...
    setAuthorizationRequired(chargerId, authRequired) {
        const data = {
    ...
    disableSmartCharging(chargerId) {
        const data = {
            smartCharging: false,
    ...
    setChargerState(chargerId, state) {
        const data = {
            enabled: state
        };
        return this.#postWaitForResponse(`/api/chargers/${chargerId}/settings`, data)
    ...
    enableIdleCurrent(chargerId, state) {
        const data = {
    ...
    lockCablePermanently(chargerId, state) {
        const data = {
    ...
    //Accept values 1-100
    ledStripBrightness(chargerId, brightness) {
        const data = {
    ...
    setDynamicChargerCurrent(chargerId, current) {
        const data = {
    ...
    setMaxChargerCurrent(chargerId, current) {
        const data = {
    ...
    setChargingPrice(siteId, currency, costPerKWh, taxPerKWh) {
        if (!isInt(siteId)) {
    ...
    setDynamicCurrentPerPhase(siteId, circuitId,
    ...
    setBasicChargePlan(chargerId, startTime, endTime, repeat, timezone) {
    ...
    deleteBasicChargePlan(chargerId) {
    ...
    getDynamicCircuitCurrent(siteId, circuitId) {
    ...
    getChargerState(chargerId) {
        return invoke(config.apiDomain, 'get', this, `/state/${chargerId}/observations?ids=${STATE_OBSERVATION_IDS.join(',')}`)
            .then(function (state) {
    ...
    getChargerSettings(chargerId) {
        return invoke(config.apiDomain, 'get', this, `/state/${chargerId}/observations?ids=${SETTINGS_OBSERVATION_IDS.join(',')}`)
            .then(function (state) {
                return state.observations || [];
            })
            .catch(reason => {
    ...
    getEqualizerSiteInfo(equalizerId) {
    ...
    getEqualizerConfig(equalizerId) {
    ...
    getLast30DaysChargekWh(chargerId) {
    ...
    getLastMonthChargekWh(chargerId) {
        let today = new Date();
    ...
    getLastChargeSessionkWh(chargerId) {
