...
const stateObservationIds = new Enum({
    'IsEnabled': 31,
...
function decodeStateObservationIds(numType) {
    return lookupEnumKey(stateObservationIds, numType);
}

function getStateObservationIds() {
    return getIdsAsArray(stateObservationIds);
}

const settingsObservationIds = new Enum({
    'DetectedPowerGridType': 21,
    'LockCablePermanently': 30,
    'EnableIdleCurrent': 37,
    'PhaseMode': 38,
    'AuthorizationRequired': 42,
    'OfflineChargingMode': 45,
    'MaxChargerCurrent': 47,
    'MaxCurrentOfflineFallback_P1': 50,
    'MaxCurrentOfflineFallback_P2': 51,
    'MaxCurrentOfflineFallback_P3': 52,
    'SoftwareRelease': 80,
...
function decodeSettingsObservationIds(numType) {
    return lookupEnumKey(settingsObservationIds, numType);
}

function getSettingsObservationIds() {
    return getIdsAsArray(settingsObservationIds);
}

function decodeEnergyChargerMode(numType) {
    switch (numType) {
        case 0: // Offline
        case 1: // Disconnected
        case 4: // Completed
        ...
        case 7: // Awaiting Authentication
        case 8: // De-authenticating
            return 'plugged_in';
        case 2: // Awaiting Start
...
const chargerMode = new Enum({
    ...
    'Car connected': 6,
    'Awaiting Authentication': 7,
    'Deauthenticating': 8
});

function decodeChargerMode(numType) {
    return lookupEnumKey(chargerMode, numType);
}

function getChargerMode() {
    return getEnumAsJson(chargerMode);
}

const commandState = new Enum({
    'Prepared': 0, //Not part of spec from Easee...
    'Sent': 1,
    'Expired': 2,
    'Executed': 3,
    'Rejected': 4
...
function decodeCommandState(numType) {
    return lookupEnumKey(commandState, numType);
}

function decodeNodeType(numType) {
    switch (numType) {
...
function decodePhaseMode(numType) {
    switch (numType) {
...
// GridType - equalizer
function decodeGridType(numType) {
    return lookupEnumKey(GRID_TYPE, numType);
}

function decodePowerGridType(numType) {
    return lookupEnumKey(DETECTED_POWER_GRID_TYPE, numType);
}

const GRID_TYPE = new Enum({
    'UNKNOWN': 0,
    'TN': 1,
    'IT': 2
});
...
const DETECTED_POWER_GRID_TYPE = new Enum({
    'NOT_YET_DETECTED': 0,
    'TN_3_PHASE': 1,
    'TN_2_PHASE_PIN_2_3_4': 2,
    'TN_1_PHASE': 3,
    'IT_3_PHASE': 4,
    'IT_1_PHASE': 5,
    // warning, wiring seems to be wrong, but charger is operational
    'WARNING_TN_2_PHASE_PIN_2_3_5': 30,
    'WARNING_TN_1_PHASE_NEUTRAL_ON_PIN_3': 31,
    // error states, will not work
    'ERROR_NO_VALID_POWER_GRID_FOUND': 50,
    'ERROR_TN_400_V_NEUTRAL_ON_WRONG_PIN': 51,
    'ERROR_IT_GND_CONNECTED_TO_PIN_2_OR_3': 52
});
...
function getIdsAsArray(enumObject) {
    let ids = [];
...
module.exports = {
    GRID_TYPE,
    DETECTED_POWER_GRID_TYPE,
    deviceTypes,
    decodeChargerMode,
    getChargerMode,
    decodeEnergyChargerMode,
    decodeNodeType,
    decodePhaseMode,
    decodeGridType,
    decodePowerGridType,
    decodeOfflineChargingModeType,
    decodeReasonForNoCurrent,
    decodeEqualizerCommandResponse,
    decodeChargerCommandResponse,
    decodeChargerObservation,
    decodeEqualizerObservation,
    decodeCommandState,
    getSettingsObservationIds,
    decodeSettingsObservationIds,
    getStateObservationIds,
    decodeStateObservationIds
}

function decodeOfflineChargingModeType(numType) {
    switch (numType) {
...
function decodeReasonForNoCurrent(numType) {
    switch (numType) {
        // charger is allocated current
        case 0: return 'Charger is OK'; break;
        case 1: return 'Max circuit current too low, adjust power circuit up'; break;
        case 2: return 'Max dynamic circuit current too low (Partner Loadbalancing)'; break;
        case 3: return 'Max dynamic offline fallback circuit current too low'; break;
        case 4: return 'Circuit fuse too low'; break;
        case 5: return 'Waiting in queue'; break;
        case 6: return 'Waiting in fully charged queue'; break;
        case 7: return 'Illegal grid type (Error - Fault in automatic grid type detection)'; break;
        case 8: return 'Primary unit has not received current request from secondary unit (car)'; break;
        case 9: return 'Master communication lost (Error)'; break;
        case 10: return 'No current from equalizer to low'; break;
        case 11: return 'No current, phase not connected'; break;
        case 25: return 'Current limited by circuit fuse'; break;
        case 26: return 'Current limited by circuit max current'; break;
        case 27: return 'Current limited by dynamic circuit current'; break;
        case 28: return 'Current limited by equalizer'; break;
        case 29: return 'Current limited by circuit load balancing'; break;
        case 30: return 'Current limited by offline settings'; break;
        case 50: return 'Secondary unit not requesting current (No car connected)'; break;
        case 51: return 'Max charger current too low'; break;
        case 52: return 'Max Dynamic charger current too low'; break;
        case 53: return 'Charger disabled'; break;
        case 54: return 'Pending scheduled charging'; break;
        case 55: return 'Pending authorization'; break;
        case 56: return 'Charger in error state'; break;
        case 57: return 'Erratic EV'; break;
        case 75: return 'Current limited by cable rating'; break;
        case 76: return 'Current limited by schedule'; break;
        case 77: return 'Current limited by charger max current'; break;
        case 78: return 'Current limited by dynamic charger current'; break;
        case 79: return 'Current limited by car not charging'; break;
        case 80: return 'Current limited by local adjustment'; break;
        case 81: return 'Current limited by car'; break;
        case 100: return 'Undefined'; break;
        default: return `UNKNOWN (${numType})`; break;
    }
}

function decodeEqualizerCommandResponse(observationId) {
...
function decodeChargerCommandResponse(observationId) {
    switch (observationId) {
        case 1: return 'Reboot'; break;
        case 2: return 'PollSingle'; break;
        case 3: return 'PollAll'; break;
...
function decodeChargerObservation(observationId) {
    switch (observationId) {
...
function decodeEqualizerObservation(observationId) {
    switch (observationId) {
        // PASSED or error codes [String]
        case 1: return 'SelfTestResult'; break;
        // JSON with details from self-test [String]
