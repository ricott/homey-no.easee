    ...
    return http.post(options)
        .then(function (result) {
            if (result.response.statusCode === 200) {
                ...
                console.log(`API call '${options.path}' failed, HTTP status code '${result.response.statusCode}', and message '${result.data}'`);
                return Promise.reject(new Error(`Token refresh failed '${result.data}'`));
            }
        })
        .catch(reason => {
            return Promise.reject(reason);
